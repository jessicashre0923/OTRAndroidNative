...
import androidCardClasses.NewAccountPinOrPasscode;
...
public class NewCardLoginUnitTest {
	...
	//GIVE YOUR TEST THE MANUAL DEFAULT DETAILS IT NEEDS TO RUN ENV IS VITAL TO RUN TEST
			...
			String TestName = "New Card Log in Unit Test ";
			String build = "Build #:  ";
		...
		try {
			...
			while (smashLogin < timesToRun) {
				...
				while (perAccountLogin < runsPerAccount) {
					...
					helpScreen.HelpScreenContinueRequired(env, storedAccount, storedPinOrPassword, accountType);
					Thread.sleep(3000);
				...
				while (companyCard.next() && companyCardloops <= smashLogin) {
					...
					androidLogs.CapturedLogs(info, " #:" + companyCardloops + " Card ID found: " + cardID);
