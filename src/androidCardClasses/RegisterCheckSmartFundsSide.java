...
public class RegisterCheckSmartFundsSide {
	...
	ScrollOptions pullToRefresh = new ScrollOptions();
	...
	public RegisterCheckSmartFundsSide registerCheckAndroid() throws Exception {
		AppStringsAndroid androidText = new AppStringsAndroid(env);
		androidLogs.setupTest("Register checks SmartFunds Side Test");
		Thread.sleep(3000);
	...
	//	ResultSet profileIDDB = dbACPTCheck.query("infx",String.format("SELECT profile_id FROM ach_profile_card_xref WHERE card_num ='"+getAccount.getCardNum()+"'"));	
		...
		pullToRefresh.Refresh(); //fails can remove it. 
